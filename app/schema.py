...
def _get_webhook_headers():
    if not WEBHOOK_ENDPOINT or not WEBHOOK_KEY:
        raise ValueError("WEBHOOK_ENDPOINT and WEBHOOK_KEY must be set for XSIAM_WEBHOOK destination.")
    return {"Authorization": WEBHOOK_KEY, "Content-Type": "application/json"}
...
class Query:
    ...
    def generate_scenario_fake_data(self, request_input: DetailedScenarioInput) -> DetailedScenarioOutput:
        ...
        for step in request_input.steps:
            ...
            for log_input in step.logs:
                ...
                logging.info(f"[XLOG-DEBUG] Raw observables from GraphQL: {observables}")
                logging.info(f"[XLOG-DEBUG] Required fields: {required_fields}")
                ...
                if observables:
                    ...
                    for key, value in observables.__dict__.items():
                        if value is not None and key in observables_init.__dict__:
                            ...
                            logging.warning(f"[XLOG-DEBUG] Observable key '{key}' NOT in Observables class, skipped!")

                    logging.info(f"[XLOG-DEBUG] Filtered observables_data: {observables_data}")
                    logging.info(f"[XLOG-DEBUG] Available Observables fields: {list(observables_init.__dict__.keys())[:50]}")
    ...
    def create_data_worker(self, request_input: DataWorkerCreateInput) -> WorkerOutput:
        ...
        if request_input.destination == "XSIAM":
            ...
            event_type = f"xlog_{request_input.type.name.lower()}"
        ...
        if scenario_tactics:
            for tactic in scenario_tactics:
                ...
                if request_input.destination == "XSIAM_WEBHOOK":
                    ...
                    event_type = f"xlog_{tactic['type'].lower()}"
                    payloads = _build_webhook_payloads(data, event_type=event_type)
            ...
            for step in scenario_steps:
                ...
                for log_input in step.logs:
                    ...
                    if request_input.destination == "XSIAM_WEBHOOK":
                        ...
                        event_type = f"xlog_{log_input.type.name.lower()}"
                        payloads = _build_webhook_payloads(data, event_type=event_type)
                        scenario_worker = WebhookSender(
                            worker_name=worker_name,
                            destination=WEBHOOK_ENDPOINT,
